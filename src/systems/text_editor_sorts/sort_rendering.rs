...
use crate::core::state::text_editor::{SortLayoutMode, SortKind};
...
pub fn render_text_editor_cursor(
    ...
    if let Some(cursor_world_pos) = calculate_cursor_visual_position(&text_editor_state, &app_state) {
        ...
        let cursor_bottom = cursor_world_pos.y + font_metrics.descender.unwrap_or(-256.0) as f32;  // Descender bottom
        ...
        // Draw circles at top
        gizmos.circle_2d(Vec2::new(cursor_world_pos.x, cursor_top), large_circle_radius, cursor_color);
        gizmos.circle_2d(Vec2::new(cursor_world_pos.x, cursor_top), medium_circle_radius, cursor_color);
        gizmos.circle_2d(Vec2::new(cursor_world_pos.x, cursor_top), small_circle_radius, cursor_color);
        
        // Draw circles at bottom
        gizmos.circle_2d(Vec2::new(cursor_world_pos.x, cursor_bottom), large_circle_radius, cursor_color);
        gizmos.circle_2d(Vec2::new(cursor_world_pos.x, cursor_bottom), medium_circle_radius, cursor_color);
        gizmos.circle_2d(Vec2::new(cursor_world_pos.x, cursor_bottom), small_circle_radius, cursor_color);
        
        debug!("Text cursor rendered at ({:.1}, {:.1}), height: {:.1}", 
        ...
        if let Some(sort) = text_editor_state.buffer.get(i) {
            ...
            if i == root_index || sort.layout_mode == SortLayoutMode::Text {
                ...
                if glyph_count == cursor_pos_in_buffer {
                    return Some(Vec2::new(root_position.x + x_offset, root_position.y + y_offset));
                }
                ...
                match &sort.kind {
                    ...
                    SortKind::Glyph { glyph_name: _, advance_width } => {
                        if glyph_count < cursor_pos_in_buffer {
