...
//! automatically discovers and displays all registered tools with proper ordering 
//! and visual feedback.

use bevy::prelude::*;
use crate::ui::theme::{
    DEFAULT_FONT_PATH, NORMAL_BUTTON, HOVERED_BUTTON, PRESSED_BUTTON,
    ...
    TOOLBAR_MARGIN, TOOLBAR_PADDING, TOOLBAR_ROW_GAP, TOOLBAR_ITEM_SPACING,
    TOOLBAR_BORDER_WIDTH,
};
use crate::ui::toolbars::edit_mode_toolbar::*;

/// Component marker for edit mode toolbar buttons
#[derive(Component)]
pub struct EditModeToolbarButton;

/// Component that identifies a tool button with its associated tool ID
#[derive(Component)]
...
///
/// This system automatically generates the toolbar UI based on all registered 
/// tools. It respects tool ordering preferences and creates interactive buttons 
/// for each tool.
...
pub fn handle_toolbar_mode_selection(
    ...
    for (interaction, _color, _border_color, tool_button, _entity) in 
        ...
        if *interaction == Interaction::Pressed {
            ...
            if current_tool.get_current() != Some(new_tool_id) {
                ...
                if let Some(current_id) = current_tool.get_current() {
                    if let Some(current_tool_impl) = 
                        tool_registry.get_tool(current_id) 
                    {
                ...
                if let Some(new_tool_impl) = 
                    tool_registry.get_tool(new_tool_id) 
                {
    ...
    {
        let is_current_tool = 
            current_tool.get_current() == Some(tool_button.tool_id);
        
        // Update button colors based on interaction state and current tool
...
/// Update text color for button children based on current tool state
