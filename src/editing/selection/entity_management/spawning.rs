            ...
            sort_query.get(active_sort_entity)
        {
            ...
            if !existing_points {
                ...
                if let Some(fontir_state) = fontir_app_state.as_ref() {
                    ...
                    spawn_fontir_points(&mut commands, sort_entity, &sort.glyph_name, position, fontir_state);
                } else if let Some(app_state) = app_state.as_ref() {
                    // Use traditional AppState spawning logic
                    spawn_appstate_points(&mut commands, sort_entity, &sort.glyph_name, position, app_state);
                } else {
...
fn spawn_fontir_points(
    ...
    if let Some(paths) = fontir_state.get_current_glyph_paths() {
        ...
        for (path_index, path) in paths.iter().enumerate() {
            ...
            for editable_point in editable_points {
                ...
                let point_world_pos = position
                    + Vec2::new(editable_point.position.x as f32, editable_point.position.y as f32);
                point_count += 1;
        ...
        warn!("[spawn_fontir_points] No FontIR paths found for glyph '{}'", glyph_name);
    }
}
...
fn spawn_appstate_points(
    ...
    if let Some(glyph_data) = app_state.workspace.font.get_glyph(glyph_name) {
        if let Some(outline) = &glyph_data.outline {
            ...
            warn!("[spawn_appstate_points] No outline found for glyph '{}'", glyph_name);
        }
    } else {
        warn!("[spawn_appstate_points] No glyph data found for '{}'", glyph_name);
    }
}
