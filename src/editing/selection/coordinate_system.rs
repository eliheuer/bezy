...
impl SelectionCoordinateSystem {
    ...
    pub fn debug_coordinate_ranges(
        ...
        let min_x = entities.iter().map(|(_, pos)| pos.x).min_by(|a, b| a.partial_cmp(b).unwrap()).unwrap();
        let max_x = entities.iter().map(|(_, pos)| pos.x).max_by(|a, b| a.partial_cmp(b).unwrap()).unwrap();
        let min_y = entities.iter().map(|(_, pos)| pos.y).min_by(|a, b| a.partial_cmp(b).unwrap()).unwrap();
        let max_y = entities.iter().map(|(_, pos)| pos.y).max_by(|a, b| a.partial_cmp(b).unwrap()).unwrap();
