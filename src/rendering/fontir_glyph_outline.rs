...
pub fn draw_bezpath_outline_at_position(
    ...
    for element in elements {
        match element {
            ...
            PathEl::LineTo(pt) => {
                let start = Vec2::new(current_pos.x as f32, current_pos.y as f32) + offset;
...
pub fn draw_bezpath_points_and_handles_at_position(
    ...
    // Then draw points
    for point in points {
        let point_pos = Vec2::new(point.position.x as f32, point.position.y as f32) + offset;
...
fn draw_cubic_curve(
    ...
    for i in 1..=SEGMENTS {
        ...
        let last_pos = Vec2::new(last_point.x as f32, last_point.y as f32) + offset;
...
fn cubic_bezier_point(start: Point, c1: Point, c2: Point, end: Point, t: f64) -> Point {
    let t2 = t * t;
    let t3 = t2 * t;
    let mt = 1.0 - t;
    let mt2 = mt * mt;
...
fn quad_bezier_point(start: Point, control: Point, end: Point, t: f64) -> Point {
    let t2 = t * t;
    let mt = 1.0 - t;
    let mt2 = mt * mt;
...
/// Draw a complete glyph outline from live Transform positions for FontIR
pub fn draw_fontir_glyph_outline_from_live_transforms(
    gizmos: &mut bevy::gizmos::gizmos::Gizmos,
    point_entities: &bevy::ecs::system::Query<
        (bevy::prelude::Entity, &bevy::transform::components::Transform, &crate::editing::selection::components::GlyphPointReference, &crate::editing::selection::components::PointType),
        bevy::ecs::query::With<crate::systems::sort_manager::SortPointEntity>,
    >,
    glyph_name: &str,
    sort_position: bevy::math::Vec2,
) {
    use crate::ui::theme::PATH_STROKE_COLOR;
    use bevy::log::debug;
    use bevy::math::Vec2;
    use std::collections::HashMap;
    
    // Group points by contour and sort by point index
    let mut contours: HashMap<usize, Vec<(Vec2, bool, usize)>> = HashMap::new();
    
    for (_entity, transform, glyph_ref, point_type) in point_entities.iter() {
        if glyph_ref.glyph_name == glyph_name {
            let world_pos = Vec2::new(transform.translation.x, transform.translation.y);
            // Convert from world position back to glyph-relative position
            let glyph_pos = world_pos - sort_position;
            let is_on_curve = point_type.is_on_curve;
            
            contours
                .entry(glyph_ref.contour_index)
                .or_default()
                .push((glyph_pos, is_on_curve, glyph_ref.point_index));
        }
    }
    
    // Sort points in each contour by their index
    for contour_points in contours.values_mut() {
        contour_points.sort_by_key(|(_, _, idx)| *idx);
    }
    
    // Draw each contour
    for (contour_index, contour_points) in contours.iter() {
        if contour_points.len() < 2 {
            continue;
        }
        
        debug!("Drawing live contour {} with {} points", contour_index, contour_points.len());
        
        // Draw lines connecting the points to reconstruct the outline
        ...
        for i in 0..len {
            ...
            let world_curr = curr_pos + sort_position;
            let world_next = next_pos + sort_position;
            
            // For now, draw simple lines between points
            // TODO: Reconstruct proper bezier curves based on point types
            if curr_on && next_on {
                // Line segment between two on-curve points
                gizmos.line_2d(world_curr, world_next, PATH_STROKE_COLOR);
            } else if curr_on && !next_on {
                // Start of a curve - draw line to control point
                // The actual curve will be drawn when we reach the next on-curve point
                let next_next_idx = (i + 2) % len;
                if next_next_idx < len {
                    let (end_pos, end_on, _) = contour_points[next_next_idx];
                    if end_on {
                        // Draw quadratic curve: curr_pos -> next_pos (control) -> end_pos
                        let world_end = end_pos + sort_position;
                        draw_quadratic_curve_gizmos(gizmos, world_curr, world_next, world_end);
                    }
                }
            }
            // Skip off-curve to on-curve connections as they're handled above
        }
    }
}

/// Draw a quadratic bezier curve using gizmos
fn draw_quadratic_curve_gizmos(
    gizmos: &mut bevy::gizmos::gizmos::Gizmos,
    start: bevy::math::Vec2,
    control: bevy::math::Vec2,
    end: bevy::math::Vec2,
) {
    use crate::ui::theme::PATH_STROKE_COLOR;
    ...
    for i in 1..=SEGMENTS {
        ...
        let point = quadratic_bezier_point_vec2(start, control, end, t);
        gizmos.line_2d(last_point, point, PATH_STROKE_COLOR);
        last_point = point;
    }
}

/// Calculate a point on a quadratic Bezier curve (Vec2 version)
fn quadratic_bezier_point_vec2(start: bevy::math::Vec2, control: bevy::math::Vec2, end: bevy::math::Vec2, t: f32) -> bevy::math::Vec2 {
    let t2 = t * t;
    let mt = 1.0 - t;
    let mt2 = mt * mt;
