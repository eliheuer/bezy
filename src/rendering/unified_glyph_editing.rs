...
pub enum UnifiedElementType {
    Point { point_entity: Entity, is_outer: bool },
    OutlineSegment,
    Handle,
}
...
pub fn render_unified_glyph_editing(
    ...
    for (sort_entity, sort, sort_transform) in active_sort_query.iter() {
        ...
        for (point_entity, point_transform, point_ref, point_type, selected) in point_query.iter() {
...
fn render_unified_handles(
    ...
    for point_data in sort_points {
        contours.entry(point_data.2.contour_index)
                .or_default()
                .push(point_data);
    }
    ...
    for (point_entity, position, _point_ref, point_type, is_selected) in sort_points {
        ...
        if point_type.is_on_curve && USE_SQUARE_FOR_ON_CURVE {
            ...
            let entity = commands.spawn((
                ...
                Transform::from_translation(position.extend(UNIFIED_POINT_Z)),
            ...
            let inner_entity = commands.spawn((
                UnifiedGlyphElement { 
                    element_type: UnifiedElementType::Point { point_entity: *point_entity, is_outer: false },
                    sort_entity,
                },
                Sprite {
                    color: point_color,
                ...
                Transform::from_translation(position.extend(UNIFIED_POINT_Z + 1.0)),
                GlobalTransform::default(),
                Visibility::Visible,
                InheritedVisibility::default(),
                ViewVisibility::default(),
        ...
        if *is_selected {
            ...
            element_entities.push(h_entity);

            // Vertical line
            let v_entity = commands.spawn((
                UnifiedGlyphElement { 
                    element_type: UnifiedElementType::Point { point_entity: *point_entity, is_outer: false },
                    sort_entity,
                },
            ...
            element_entities.push(v_entity);
        }
    }
}
