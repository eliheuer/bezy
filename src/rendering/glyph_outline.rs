...
/// Draw a complete glyph outline at a specific position with cubic curves
...
pub fn draw_glyph_points_at_position(
    ...
    for contour in &outline.contours {
        ...
        for point in &contour.points {
            ...
            let is_on_curve = match &point.point_type {
                PointTypeData::Move | PointTypeData::Line | PointTypeData::Curve => true,
                _ => false,
            };
...
pub fn draw_control_handles_at_position(
    ...
    for contour in &outline.contours {
        draw_contour_handles_at_position(gizmos, viewport, contour, offset);
    }
}
...
fn draw_curve_segment_at_position(
    ...
    for i in 0..points.len() - 1 {
        let start_pos = viewport.to_screen(DPoint::from((
            points[i].x as f32 + offset.x,
            points[i].y as f32 + offset.y,
        )));
        let end_pos = viewport.to_screen(DPoint::from((
