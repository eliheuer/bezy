...
pub fn render_mesh_glyph_outline(
    ...
    point_query: Query<
        (Entity, &Transform, &crate::editing::selection::components::GlyphPointReference, &crate::editing::selection::components::PointType),
    ...
    for (sort_entity, sort, transform) in active_sort_query.iter() {
        ...
        let has_visible_points = point_query.iter().any(|(_, _, point_ref, _)| {
...
pub fn create_line_mesh(start: Vec2, end: Vec2, width: f32) -> Mesh {
    ...
    let vertices = vec![
        [start_rel.x - perpendicular.x, start_rel.y - perpendicular.y, 0.0], // Bottom left
        [start_rel.x + perpendicular.x, start_rel.y + perpendicular.y, 0.0], // Top left  
        [end_rel.x + perpendicular.x, end_rel.y + perpendicular.y, 0.0],     // Top right
        [end_rel.x - perpendicular.x, end_rel.y - perpendicular.y, 0.0],     // Bottom right
    ];
...
fn render_fontir_outline(
                ...
                kurbo::PathEl::CurveTo(c1, c2, pt) => {
                    if let Some(start) = current_pos {
                        ...
                        for i in 1..=segments {
                            ...
                            let c1_pos = Vec2::new(c1.x as f32, c1.y as f32) + position;
                            let c2_pos = Vec2::new(c2.x as f32, c2.y as f32) + position;
                            let curve_pos = Vec2::new(
                                mt3 * start.x + 3.0 * mt2 * t * c1_pos.x + 
                                3.0 * mt * t2 * c2_pos.x + t3 * end.x,
                                mt3 * start.y + 3.0 * mt2 * t * c1_pos.y + 
                                3.0 * mt * t2 * c2_pos.y + t3 * end.y,
                            );
                            
                            let entity = spawn_line_mesh(
                                commands, meshes, materials,
                                last_pos, curve_pos, PATH_LINE_WIDTH,
                                PATH_STROKE_COLOR, GLYPH_OUTLINE_Z,
                                sort_entity, OutlineElementType::PathSegment
                            );
                            segment_entities.push(entity);
                ...
                kurbo::PathEl::QuadTo(c, pt) => {
                    if let Some(start) = current_pos {
                        ...
                        for i in 1..=segments {
                            ...
                            let c_pos = Vec2::new(c.x as f32, c.y as f32) + position;
                            let curve_pos = Vec2::new(
                                mt * mt * start.x + 2.0 * mt * t * c_pos.x + t * t * end.x,
                                mt * mt * start.y + 2.0 * mt * t * c_pos.y + t * t * end.y,
                            );
    ...
    outline_entities.path_segments.insert(sort_entity, segment_entities);
}
...
fn render_fontir_outline_live(
    ...
    if let Some(original_paths) = fontir_state.get_glyph_paths_with_edits(glyph_name) {
        let mut segment_entities = Vec::new();
            ...
            for element in elements {
                match element {
                    kurbo::PathEl::MoveTo(_) => {
                        ...
                        if let Some((live_pos, _)) = live_positions.get(&(contour_idx, element_point_index)) {
                            current_pos = Some(*live_pos + sort_position);
                        } else if let kurbo::PathEl::MoveTo(pt) = element {
                            current_pos = Some(Vec2::new(pt.x as f32, pt.y as f32) + sort_position);
                        }
                    ...
                    kurbo::PathEl::LineTo(_) => {
                        if let Some(start) = current_pos {
                            ...
                            let end = if let Some((live_pos, _)) = live_positions.get(&(contour_idx, element_point_index)) {
                    ...
                    kurbo::PathEl::CurveTo(_, _, _) => {
                        if let Some(start) = current_pos {
                            ...
                            let cp1 = if let Some((live_pos, _)) = live_positions.get(&(contour_idx, element_point_index)) {
                                *live_pos + sort_position
                            } else if let kurbo::PathEl::CurveTo(c1, _, _) = element {
                                Vec2::new(c1.x as f32, c1.y as f32) + sort_position
                            } else { start };
                            
                            let cp2 = if let Some((live_pos, _)) = live_positions.get(&(contour_idx, element_point_index + 1)) {
                                *live_pos + sort_position
                            } else if let kurbo::PathEl::CurveTo(_, c2, _) = element {
                                Vec2::new(c2.x as f32, c2.y as f32) + sort_position
                            } else { start };
                            
                            let end = if let Some((live_pos, _)) = live_positions.get(&(contour_idx, element_point_index + 2)) {
                                *live_pos + sort_position
                            ...
                            for i in 1..=segments {
                                ...
                                let curve_pos = Vec2::new(
                                    mt3 * start.x + 3.0 * mt2 * t * cp1.x + 
                                    3.0 * mt * t2 * cp2.x + t3 * end.x,
                                    mt3 * start.y + 3.0 * mt2 * t * cp1.y + 
                                    3.0 * mt * t2 * cp2.y + t3 * end.y,
                                );
                    ...
                    kurbo::PathEl::QuadTo(_, _) => {
                        if let Some(start) = current_pos {
                            ...
                            let cp = if let Some((live_pos, _)) = live_positions.get(&(contour_idx, element_point_index)) {
                                *live_pos + sort_position
                            } else if let kurbo::PathEl::QuadTo(c, _) = element {
                                Vec2::new(c.x as f32, c.y as f32) + sort_position
                            } else { start };
                            
                            let end = if let Some((live_pos, _)) = live_positions.get(&(contour_idx, element_point_index + 1)) {
                                *live_pos + sort_position
...
fn render_ufo_outline_live(
    ...
    for (contour_idx, contour) in outline.contours.iter().enumerate() {
        ...
        render_contour_path_meshes_live(
            commands, meshes, materials, &mut path_entities,
            sort_entity, contour, sort_position, &live_positions, contour_idx
        );
...
fn render_contour_path_meshes_live(
    ...
    let segments = extract_path_segments_live(contour, live_positions, contour_idx);
    ...
    for segment in segments {
        match segment {
            ...
            PathSegmentLive::Curve { start, control1, control2, end } => {
                // Tessellate curve into multiple line segments
                let start_pos = start + sort_position;
...
fn render_contour_handles_meshes_live(
    ...
    for (i, point) in points.iter().enumerate() {
        if !is_on_curve(point) {
            ...
            let handle_pos = if let Some(live_pos) = live_positions.get(&(contour_idx, i)) {
                *live_pos + sort_position
            } else {
            ...
            if let Some(prev_idx) = prev_on_curve {
                let prev_pos = if let Some(live_pos) = live_positions.get(&(contour_idx, prev_idx)) {
                    *live_pos + sort_position
                } else {
                    Vec2::new(points[prev_idx].x as f32, points[prev_idx].y as f32) + sort_position
                };
                let entity = spawn_line_mesh(
                    commands, meshes, materials,
                    prev_pos, handle_pos, HANDLE_LINE_WIDTH,
            ...
            if let Some(next_idx) = next_on_curve {
                let next_pos = if let Some(live_pos) = live_positions.get(&(contour_idx, next_idx)) {
                    *live_pos + sort_position
                } else {
                    Vec2::new(points[next_idx].x as f32, points[next_idx].y as f32) + sort_position
                };
                let entity = spawn_line_mesh(
                    commands, meshes, materials,
                    handle_pos, next_pos, HANDLE_LINE_WIDTH,
...
enum PathSegmentLive {
    ...
    Curve { start: Vec2, control1: Vec2, control2: Vec2, end: Vec2 },
}
...
fn extract_path_segments_live(
    ...
    for i in 0..points.len() {
        ...
        let current_pos = live_positions.get(&(contour_idx, i))
            .copied()
            .unwrap_or_else(|| Vec2::new(current.x as f32, current.y as f32));
        let next_pos = live_positions.get(&(contour_idx, (i + 1) % points.len()))
            .copied()
